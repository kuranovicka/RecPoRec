...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /** Temelj za audio knjige (folder zvučnih fajlova zipovan i uvezen kao "knjiga") -
         * ne dira postojeće podatke, samo dodaje tri nove kolone sa bezopasnim
         * podrazumevanim vrednostima. Postojeći (tekstualni) dokumenti ih jednostavno
         * ne koriste. */
        private val MIGRATION_11_12 = object : Migration(11, 12) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE documents ADD COLUMN audioFileIndex INTEGER NOT NULL DEFAULT 0")
                db.execSQL("ALTER TABLE documents ADD COLUMN audioPositionMs INTEGER NOT NULL DEFAULT 0")
                db.execSQL("ALTER TABLE documents ADD COLUMN audioDurationMs INTEGER NOT NULL DEFAULT 0")
            }
        }

        /** Oznake u audio knjizi sad pamte poziciju (fajl + milisekunda) umesto pozicije
         * karaktera - ne dira postojece (tekstualne) oznake, samo dodaje dve nove kolone sa
         * bezopasnim podrazumevanim vrednostima. */
        private val MIGRATION_12_13 = object : Migration(12, 13) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE bookmarks ADD COLUMN audioFileIndex INTEGER NOT NULL DEFAULT -1")
                db.execSQL("ALTER TABLE bookmarks ADD COLUMN audioPositionMs INTEGER NOT NULL DEFAULT 0")
            }
        }
